...
; Flocking and mult-turtle behaviour is derived from the Flocking Vee Formations sample netlogo code.


globals [ q ; q is the probability that the buttefly moves directly to the highest surrounding patch
          UPDRAFT-DISTANCE
          BASE-SPEED
          SPEED-CHANGE-FACTOR
          VISION-DISTANCE
...
to setup
  ca
  set year 0
  file-open (word "/media/alex/AJMCVITT/nlogo/" start-date ".txt") ; Open the IO stream for the initial surface data
  ...
  [
    ...
    ifelse climate != "None"[
      ifelse climate = "Additive"[
        ask patch next-X next-Y [set elevation next-elevation + climate-change-additive]
      ][
        ask patch next-X next-Y [set elevation next-elevation * climate-change-multiply]
      ]
      ][
  ...
  ask patches [
    set pcolor scale-color orange elevation -30 110
  ]
  ; Create five birds
  if startx = 0 [set startx 146]
  if starty = 0 [set starty -67]
  crt 1
  [
    set size 5
  ...
  set q 0.6
  ; Variables from the Flocking Vee Formations interface.
  set UPDRAFT-DISTANCE 4
  set BASE-SPEED 0.2
  set SPEED-CHANGE-FACTOR 0.15
  set VISION-DISTANCE 50
  set OBSTRUCTION-CONE 45
  set MAX-TURN 2
  set TOO-CLOSE 1.6
  set SHOW-UNHAPPY? false
  set VISION-CONE 119
...
to go ; This is the master schedule
  ...
  if change-surface [
    ...
    [
      ...
      set filen ((round (ticks / movement-between-date-changes) + start-date) - (year * 365))
      file-open (word "/media/alex/AJMCVITT/nlogo/"  filen ".txt") ; Read in the next days elevation file
      while [not file-at-end?]
      [
        let next-X file-read
        let next-Y file-read
...
;; Citation information for the flocking model!
...
417
Current date
round ticks / movement-between-date-changes
17
1
11
