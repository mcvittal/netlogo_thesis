...
globals [
          ...
          past-turtle-locations ; Storing the locations of all turtles in the past tick only
          full-migration ; Becomes true if all birds are migrated
          PRINTER ; variable to print what type of headless we are on
]
patches-own [
  temperature
]
...
to setup
  ...
  set past-turtle-locations []
  ; Add birds to surface - amount specified by slider
  crt number-of-birds
  [
    set size 5
    ; set initial location of butterflies
    let starting-x (random-range 130 145)
    let starting-y (random-range 230 245)

    set all-turtle-locations lput (list starting-x starting-y -9999) all-turtle-locations
    set past-turtle-locations lput (list starting-x starting-y -9999) past-turtle-locations
...
to read-surface [julian-date]
  ...
  if julian-date > 365 [set julian-date julian-date - 364]
  ; Read in initial surface data
  ifelse file-exists? "/mnt/disk2/nlogo/0.txt"[
    file-open (word "/mnt/disk2/nlogo/" julian-date ".txt") ; Open the IO stream for the initial surface data
  ][file-open (word "/scratch-reserved/ajmcvitt/nlogo/" julian-date ".txt") ; Open the IO stream for the initial surface data
  ]
  ;show julian-date
  while [not file-at-end?]
  [ let next-X file-read
    let next-Y file-read
    let next-temperature 0
    carefully
    [ set next-temperature file-read ]
    [ set next-temperature -9999]
    if next-temperature > 200 [set next-temperature -9999]
    let loc 0
...
to go ; This is the master schedule
  ...
  [ if ticks mod movement-between-date-changes = 0
    [
      let filen ((start-date + round ticks / movement-between-date-changes) mod 364)
  ...
  [ ifelse temperature < max-surface
    [ ifelse track-movement
      ...
      if any? other turtles-here [
        let target max-one-of patches with [not any? turtles-here] [distance myself]
        face target
        fd 1
      ]
      foreach all-turtle-locations
      [ x -> (reassign-patches (item 0 x) (item 1 x) (item 2 x))]
      set past-turtle-locations lput (list xcor ycor this-elev) past-turtle-locations
...
@#$#@#$#@
<experiments>
  <experiment name="baseline_nocc" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>past-turtle-locations</metric>
    <enumeratedValueSet variable="write-path-to-file">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="climate-change-exponent">
      <value value="0.9"/>
    </enumeratedValueSet>
    ...
    <enumeratedValueSet variable="number-of-birds">
      <value value="2"/>
      <value value="3"/>
      <value value="4"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="PRINTER">
      <value value="&quot;Baseline&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="additive_cc" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>past-turtle-locations</metric>
    <enumeratedValueSet variable="write-path-to-file">
  ...
  <experiment name="multiply_cc" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>past-turtle-locations</metric>
    <enumeratedValueSet variable="write-path-to-file">
  ...
  <experiment name="exponent_cc" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>past-turtle-locations</metric>
    <enumeratedValueSet variable="write-path-to-file">
