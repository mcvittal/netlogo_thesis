...
to setup
  ca
  file-open "/media/alex/AJMCVITT/nlogo/30.txt" ; Open the IO stream for the initial surface data
...
to go ; This is the master schedule
  ...
  tick
  file-open (word "/media/alex/AJMCVITT/nlogo/" ticks ".txt") ; Read in the next days elevation file
  while [not file-at-end?]
  [
    let next-X file-read
    let next-Y file-read
